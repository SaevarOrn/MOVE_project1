...
\begin{homeworkProblem}
    ...
    In this first task, we implement a scheduler that schedules a set of processes, $P_1,P_2,\dots,P_n$, such that the processes start in cyclical order, $P_{i+1}$ starts after $P_i$ for each $1\le i<n$ and $P_{1}$ starts after $P_n$.
    
    The scheduler is made up of $n$ cells, where cell $i$ controls process $P_i$. A cell contains $4$ ports it uses for comunication. $in$ and $out$ ports are used to comunicate with other cells in the scheduler and $a$ and $b$ are used to signal the process to start and end execution. Each cell must first wait for a signal on it's $in$ port before starting it's process. It will then signal the next cell to start and signal that it process has ended but we do not make any constraints on the order these two actions are performed in. Each cell is defined using the following template:
    \[
        P = \overline{in}.a.(out.b.P + b.out.P)
    \]
    ...
    \[
        S = (P_1|P_2|\dots|P_n|c_n.nil)\backslash\{c_i|1\le i\le n\}
    \]
    ...
    \perlscript{../task1.ccs}{Scheduler .CCS file (task1.ccs)}
    ...
    \]
    Since we define $\psi$ using strong until, it also verifies that the scheduler actually performs $a_1$ at some point.
    ...
    We now define a property that verifies that each cell in the scheduler alternates between $a$ and $b$ moves. Again we break this formula down into smaller units. We want to show that whenever a cell makes an $a$ move, it will not make an $a$ move again until it has made a $b$ move, and vice versa. We also want to make sure that each cell at some point makes either an $a$ or a $b$ move. We define the following formulas:
    \begin{align*}
    ...
    \[
        alternate = \bigwedge\limits_{i=1}^n(\phi_i\land \psi_i\land\gamma_i)
    \]
...
\newpage % new page here since listing is just over one page anyway

	\perlscript{../task1.mu}{Scheduler .MU file (task1.mu)}
...
\perlscript{../task2.ccs}{1-Dimensional Solitaire .CCS file (task2.ccs)}
  Each template behaves differently based on what color it represents. Black tiles can jump either one or two tiles to the right depending on whether the $Empty$ tile is one or two tiles to the right (white tiles mirror this functionality in the opposite direction). Recall that $Empty$ tiles can swap with any adjacent tile.
	\emph{(Note: The model does not explicitly disallow a process from jumping over an identical process to swap with the empty process. This is however no issue, since would be trivial to prove that this is equivalent to two legal moves. Those two legal moves are of course swapping the empty tile twice \-- this works because there is no swapping the position of two same-colored processes has no effect.)}
...
\perlscript{../task2.mu}{1-Dimensional Solitaire .MU file (task2.mu)}

  This elegant solution works due to the way that the game was modelled.
	Colored tiles (\(B_i, W_i\)) can always indicate what color occupies tile $i$ since $proc ~ Board$ does not restrict on $w_i$ or $b_i$ (for any $i$).
    ...
    \begin{align*}
        \texttt{cwb-nc>}&~\texttt{load task2.ccs}   \\
        \texttt{cwb-nc>}&~\texttt{load task2.mu}    \\
        \texttt{cwb-nc>}&~\texttt{chk Board is\_solvable}     \\
    \end{align*}
