...
\begin{homeworkProblem}
    ...
    In this problem, we implement a scheduler that schedules processes, $P_1,P_2,\dots,P_n$, such that they are scheduled in cyclical order, $P_i$ comes before $P_{i+1}$ and $P_{1}$ comes after $P_n$.
    ...
    We can also verify that the two properties hold by encoding them in as recursive Hennessy-Milner Logic formulas and verifying that the scedule satisfies these formulas.

    We begin with the cyclical property. We can break it down into a smaller problems. We must verify that every time the scheduler performs action $a_i$, then the next $a$ action it performs is $a_{i+1}$, for each $1\le i\le n$ where $a_{n+1} = a_1$. Another way to frase this is that every time the scheduler makes an $a_i$ move, then it does not make an $a$ move until it makes an $a_{i+1}$ move. This property can be encoded using computational tree logic:
    \begin{align*}
...
\perlscript{../task2.mu}{1-Dimensional Solitaire .MU file}
  ...
  This elegant solution works due to the way that the game was modelled.
	...
	Using the fixed point-notation of Hennessy–Milner logic, we simply ask if the state is reachable from the start configuration:
	\[
		\$~cwb-nc> ~ chk~Board~is\_solvable
	\]
